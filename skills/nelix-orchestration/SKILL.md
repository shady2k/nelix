...
The user can answer any or all, in any order. Answer with `nelix_respond(session_id, answer, decision_id)` — always pass the `decision_id` from the status read (several decisions can be pending at once; the id lands your answer on the one you read).
