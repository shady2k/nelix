...
def test_env_cmd_failure_returns_502_redacted(monkeypatch, tmp_path):
    spec = make_spec(command="claude", args=["--foo"], driver="claude",
                     env_cmd={"TOK": "echo LEAKOUT; echo LEAKERR 1>&2; exit 5"})
    srv, mgr, buf = _serve(monkeypatch, tmp_path, spec, port=8831)
    try:
...
def test_timeout_env_cmd_returns_502(monkeypatch, tmp_path):
    spec = make_spec(command="claude", args=["--foo"], driver="claude",
                     env_cmd={"SLOW": "sleep 5"}, env_cmd_timeout_seconds=0.2)
    srv, mgr, buf = _serve(monkeypatch, tmp_path, spec, port=8832)
    try:
