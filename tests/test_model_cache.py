...
def test_hit_avoids_second_fetch():
    calls = []
    c = _cache(calls)
    a = c.models("zai", "https://b/", "bearer", "tok", {}, "anthropic")
    b = c.models("zai", "https://b/", "bearer", "tok", {}, "anthropic")
    assert a == b == LIST
...
def test_force_refetches():
    calls = []
    c = _cache(calls)
    c.models("zai", "https://b", "bearer", "tok", {}, "anthropic")
    c.models("zai", "https://b", "bearer", "tok", {}, "anthropic", force=True)
    assert len(calls) == 2
...
def test_different_token_is_different_key():
    calls = []
    c = _cache(calls)
    c.models("zai", "https://b", "bearer", "tokA", {}, "anthropic")
    c.models("zai", "https://b", "bearer", "tokB", {}, "anthropic")   # visibility is credential-scoped
    assert len(calls) == 2
...
def test_single_flight_coalesces_concurrent_cold_calls():
    ...
    outs = []
    def worker():
        outs.append(c.models("zai", "https://b", "bearer", "tok", {}, "anthropic"))
