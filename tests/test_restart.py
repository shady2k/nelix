...
def test_restart_active_session_replaces_at_full_capacity(tmp_path, monkeypatch):
    ...
    _out = mgr.start("claude", "task A", str(tmp_path), owner_id=OWNER); sid = _out.session_id
    out = mgr.restart(sid, owner_id=OWNER)                                  # at full cap (limit 1)
...
def test_restart_gone_session_resolves_from_persisted_meta(tmp_path, monkeypatch):
    ...
    _out = mgr.start("claude", "task B", str(tmp_path), owner_id=OWNER); sid = _out.session_id
    ...
    out = mgr.restart(sid, owner_id=OWNER)
    assert out.status == "restarted"
    assert mgr.get(out.session_id).task == "task B"


def test_restart_unknown_session(tmp_path, monkeypatch):
    mgr = _mgr(tmp_path, monkeypatch)
    assert mgr.restart("s-nope", owner_id=OWNER).status == "unknown_session"
...
def test_restart_budget_exhausted_then_force_resets(tmp_path, monkeypatch):
    ...
    o1 = mgr.restart(sid, owner_id=OWNER); assert o1.status == "restarted" and o1.restart_count == 1
    o2 = mgr.restart(o1.session_id, owner_id=OWNER); assert o2.status == "restarted" and o2.restart_count == 2
    o3 = mgr.restart(o2.session_id, owner_id=OWNER)
    assert o3.status == "restart_budget_exhausted" and o3.max_restarts == 2
    o4 = mgr.restart(o2.session_id, force=True, owner_id=OWNER)
...
def test_restart_budget_is_per_lineage(tmp_path, monkeypatch):
    ...
    _outa = mgr.start("claude", "A", str(tmp_path), owner_id=OWNER); a = _outa.session_id
    _outb = mgr.start("claude", "B", str(tmp_path), owner_id=OWNER); b = _outb.session_id
    oa = mgr.restart(a, owner_id=OWNER); assert oa.status == "restarted"        # lineage A: count 1
    ob = mgr.restart(b, owner_id=OWNER); assert ob.status == "restarted"        # lineage B independent: count 1
    assert mgr.restart(oa.session_id, owner_id=OWNER).status == "restart_budget_exhausted"
    assert mgr.restart(ob.session_id, owner_id=OWNER).status == "restart_budget_exhausted"
...
def test_restart_balances_reserved_counter(tmp_path, monkeypatch):
    ...
    mgr = _mgr(tmp_path, monkeypatch, limit=1)
    _out = mgr.start("claude", "t", str(tmp_path), owner_id=OWNER); sid = _out.session_id
    out = mgr.restart(sid, owner_id=OWNER)                          # active path (reserve)
