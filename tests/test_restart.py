...
def test_restart_unknown_session(tmp_path, monkeypatch):
    ...
    assert mgr.restart("s-nope").status == "unknown_session"
...
def test_restart_budget_exhausted_then_force_resets(tmp_path, monkeypatch):
    ...
    o4 = mgr.restart(o2.session_id, force=True)
...
def test_restart_budget_is_per_lineage(tmp_path, monkeypatch):
    ...
    assert mgr.restart(ob.session_id).status == "restart_budget_exhausted"
...
def test_restart_balances_reserved_counter(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="concurrency_limit=1 reached"):
        mgr.start("claude", "other", str(tmp_path))
