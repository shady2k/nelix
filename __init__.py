...
def register(ctx):
    ...
    ctx.register_tool(
        ...
        {"description": (
            ...
            " your answer as a fresh follow-up turn, not a keystroke into a paused modal. If the agent"
            " is still busy when your answer lands (the usual case), expect `status:\"queued\"`: it was"
            " accepted and will be delivered when the agent next goes idle — `next_action` is"
            " `refresh_status`, not `end_turn`. If instead the agent already finished before your answer"
            " got there, expect `status:\"not_delivered\"` (`reason:\"executor_finished\"`) instead of"
            " `resumed`: nothing was sent — its `next_action` is also `refresh_status`, so reconcile via"
            " nelix_status and report the agent's actual outcome to the user."
            " The returned result is the COMPLETE outcome of this call — obey its `next_action`"
            " (`end_turn` → stop and wait to be woken; `report` → relay to the user;"
            " `ask_user`/`fix_call`/`recover`/`refresh_status` → act accordingly)."
            " Do NOT call nelix_status after this."),
